    ...
    <section className="flex flex-col md:flex-row gap-1 md:gap-8 p-2 justify-center">
      ...
      <div className="basis-3/4">
        ...
        <div className="flex flex-wrap gap-4 w-full items-center invisible md:visible">
          <img className="h-fit w-[180px]" src={ProductInfo?.image ? `http://localhost:8000/${ProductInfo.image}` : kopidetail} alt="Detail Kopi 1" />
          <img className="h-fit w-[180px]" src={ProductInfo?.image ? `http://localhost:8000/${ProductInfo.image}` : kopidetail} alt="Detail Kopi 2" />
          <img className="h-fit w-[180px] object-cover" src={ProductInfo?.image ? `http://localhost:8000/${ProductInfo.image}` : kopidetail} alt="Detail Kopi 3" />
        </div>
      </div>
      <div className="flex flex-col gap-2 basis-2/4">
        <div className="gap-2 flex flex-col gap-2 md:gap-8">
          ...
          <div className="md:font-bold text-2xl font-semibold md:text-5xl">{ProductInfo?.product_name}</div>
          ...
          <div className="flex flex-row border-2 rounded-lg items-center w-fit h-fit">
            <button className="border-2 hover:bg-oren text-lg md:text-3xl text-black p-2 md:p-4 border-oren rounded-lg" onClick={handleDecrement} >
              -
            </button>
            <div className="text-lg p-2 md:text-3xl text-black md:p-4">{quantity}</div>
            <button className="border-2 hover:bg-oren text-lg md:text-3xl text-black p-2 md:p-4 border-oren rounded-lg" onClick={handleIncrement} >
              +
            </button>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-2 md:justify-between">
            <button onClick={() => {}} className="p-4 font-semibold bg-orange-400 rounded-xl md:py-2 md:px-28 w-full text-2xl text-black">Buy</button>
